...
const Terminal = () => {
  ...
  useEffect(() => {
    async function listenDbInitialization() {
      async function loadData() {
        ...
        //: {developerSettings:boolean, test:string, threads:number} | null 
  ...
  async function startSearch(type: number) {
    ...
    if (!ref.current) {
      counter++
      checks.set(counter, { id: counter, message: "Number of Threads: ", lineType: 3, messageOptional: settingsDataRef.current.threads, result: true } as Initialization);
      setHashmap(new Map(hashmap));
    } else {
      setStopScrape(false)
      return
    ...
    if (!ref.current) {
      counter++
      checks.set(counter, { id: counter, message: "Number of Files: ", lineType: 3, messageOptional: numOfFiles, result: true } as Initialization);
      setHashmap(new Map(hashmap));
    } else {
      setStopScrape(false)
      return
