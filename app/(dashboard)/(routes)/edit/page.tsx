        ...
        <div>
          ...
          {displaySessions ? <div>
            {sessions.map((session, index) => (
              <DialogSessions msg={session.path + " - " + session.total_files + " file(s)"} title={formatDate(session.date) + " - Session " + session.session_number} href={`/edit/editPage?directory=${session.path}&totalSongs=${totalSongs}&pageNo=${1}&pageSize=${10}&session=${session.table_name}`} variant="none" type={false} />
            ))}
          </div> : null}
        </div>
      </div>
